...
<body>
    ...
    <nav class="navbar navbar-light bg-light justify-content-between">
        <b></b>
        <form class="form-inline" action="alinankitap.php" method="post">
            <input class="form-control mr-sm-2" type="search" name="kelime" onkeypress="return isNumberKey(event)" placeholder="Bir Şeyler Yazın" maxlength="9"
                aria-label="Search">
                <script>
    ...
    if (!$kelime) {
       ...
       -----  Arama Yapabilmek İçin Birşeyler Yazın -----
        ...
        if (strlen($kelime)<9 ) {
            ...
            -----  ÖĞRENCİ NUMARASI MAKSİMUM 9 RAKAMDAN OLUŞMALIDIR -----
          </div>';
        }else {
           
                $sorgu =$db ->prepare('SELECT * FROM kullanicilar  WHERE ogrenci_no  LIKE  :ogrenci_no');
                $sorgu ->execute(array(':ogrenci_no'=> '%'.$kelime.'%'));
            ...
            if ($sorgu -> rowCount()) {
                foreach ($sorgu as $row) {
                    ...
                    $row['ogrenci_ad'] .' Aldığı Kitabı daha bırakmadı</b>
                  </div>';
